    ...
    <div class="container mx-auto px-4 py-8">
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <h1 class="text-2xl font-bold">Stage {{ sessionStore.sessionStage }}</h1>
            ...
            <div class="flex space-x-4 mt-4">
                <button v-if="canMovePrevious" @click="handleMove('previous')" :disabled="sessionStore.loading"
                    class="px-4 py-2 bg-gray-500 text-white rounded">
                    Previous Stage
                </button>

                <button v-if="canMoveNext" @click="handleMove('next')" :disabled="sessionStore.loading"
        ...
        <StageContent :stage="sessionStore.sessionStage" :data="{
            stage: sessionStore.sessionStage
...
const username = computed(() => sessionStore.user?.username);
const logout = () => {
    sessionStore.logout()
    router.push('/logout')
}
...
const handleMove = async (direction: 'next' | 'previous') => {
    try {
        ...
        await router.push(`/stage/${newStage}`);
    } catch (error) {
